...
sub _load_unique_bits{
	...
	if( $sheet_ref ){
		my $x = 0;
		for my $header ( @{$sheet_ref->{list_keys}} ){
			###LogSD	$phone->talk( level => 'debug', message => [
			###LogSD		"processing header: $header" ] );
			if( exists $method_lookup->{$header} ){
				my $method = $method_lookup->{$header};
				###LogSD	$phone->talk( level => 'trace', message => [
				###LogSD		"Implementing -$method- with value: $sheet_ref->{list}->[$x]->{raw_text}" ] );
				$self->$method( $sheet_ref->{list}->[$x]->{raw_text} );
			}
			$x++;
		}
	}
