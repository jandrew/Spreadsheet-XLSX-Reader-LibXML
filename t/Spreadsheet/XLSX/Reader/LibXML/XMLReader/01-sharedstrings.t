...
ok			$capture = $test_instance->get_shared_string_position( 0 ),
...
ok			$capture = $test_instance->get_shared_string_position( 15 ),
...
is			$_, $answer_ref->[$answer_row++]->[$x],
			...
			my	$row = 10;
ok			$capture = $test_instance->get_shared_string_position( $row ),
...
is			$test_instance->_get_count, $answer_ref->[1],
										"Check for correct count";
while( 	$capture = $test_instance->get_shared_string_position ){
				$x = 0;
				@answer = split "\n", $capture;
map{
is			$_, $answer_ref->[$answer_row]->[$x],
