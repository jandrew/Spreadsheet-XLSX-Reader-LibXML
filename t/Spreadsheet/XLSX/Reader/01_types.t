...
my			$question_ref =[
				...
				1900, 1904, 1970,
				[{
					translation			=> FourteenFromWinExcelNum,
					borderId			=> 5,
					fillId				=> 0,
					font				=> bless( {},'XML::LibXML::Element'),
					xfId				=> 10,
					applyNumberFormat	=> 1,
					pivotButton			=> undef,
					applyAlignment		=> undef,
					applyFont			=> undef,
				}],
				[{
					translation			=> FourteenFromWinExcelNum,
				...
				{ number => FourteenFromAppleExcelNum, s =>ZeroFromNum },
			];
my			$bad_value_ref =[
				'badfile.not',
				'badfile.not',
...
is			ZeroFromNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform ZeroFromNum on (test $position): " . ($question_ref->[$position++] // 'Undef');
			}( 0..3 );
			map{
is			OneFromNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform OneFromNum on (test $position): " . ($question_ref->[$position++] // 'Undef');
			}( 0..4 );
			map{
is			TwoFromNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform TwoFromNum on (test $position): " . ($question_ref->[$position++] // 'Undef');
			}( 0..3 );
			map{
is			ThreeFromNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform ThreeFromNum on (test $position): " . ($question_ref->[$position++] // 'Undef');
			}( 0..3 );
			map{
is			FourFromNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform FourFromNum on (test $position): " . ($question_ref->[$position++] // 'Undef');
			}( 0..4 );
			map{
is			NineFromNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform NineFromNum on (test $position): " . ($question_ref->[$position++] // 'Undef');
			}( 0..4 );
			map{
is			TenFromNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform TenFromNum on (test $position): " . ($question_ref->[$position++] // 'Undef');
			}( 0..4 );
			map{
like		ElevenFromNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform ElevenFromNum on (test $position): " . ($question_ref->[$position++] // 'Undef');
			}( 0..0 );
			map{
is			TwelveFromNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform TwelveFromNum on (test $position): " .
...
is			FourteenFromWinExcelNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform FourteenFromWinExcelNum on (test $position): " . ($question_ref->[$position] // 'Undef');
			};
...
is			FourteenFromAppleExcelNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform FourteenFromAppleExcelNum on (test $position): " . ($question_ref->[$position++] // 'Undef');
			}( 0..6 );
			map{
			$capture = capture_stderr{
is			FifteenFromWinExcelNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform FifteenFromWinExcelNum on (test $position): " . ($question_ref->[$position] // 'Undef');
			};
like		$capture, $answer_ref->[($position+2)],
							"... and check the error message";
			$position++;
...
is			FifteenFromAppleExcelNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform FifteenFromAppleExcelNum on (test $position): " . ($question_ref->[$position++] // 'Undef');
			}( 0..6 );
			map{
			$capture = capture_stderr{
is			SixteenFromWinExcelNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform SixteenFromWinExcelNum on (test $position): " . ($question_ref->[$position] // 'Undef');
			};
like		$capture, $answer_ref->[($position+2)],
							"... and check the error message";
			$position++;
...
is			SixteenFromAppleExcelNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform SixteenFromAppleExcelNum on (test $position): " . ($question_ref->[$position++] // 'Undef');
			}( 0..6 );
			map{
			$capture = capture_stderr{
is			SeventeenFromWinExcelNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform SeventeenFromWinExcelNum on (test $position): " . ($question_ref->[$position] // 'Undef');
			};
like		$capture, $answer_ref->[($position+2)],
							"... and check the error message";
			$position++;
...
is			SeventeenFromAppleExcelNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform SeventeenFromAppleExcelNum on (test $position): " . ($question_ref->[$position++] // 'Undef');
			}( 0..6 );
			map{
is			EighteenFromNum->( $question_ref->[$position] ), $answer_ref->[$position],
							"Check the result of the transform EighteenFromNum on (test $position): " . ($question_ref->[$position++] // 'Undef');
			}( 0..8 );
			map{
ok			EpochYear->( $question_ref->[$position] ),
							"Verifying the identification of valid EpochYear on (test $position): " . ($question_ref->[$position++] // 'Undef');
			}( 0..1 );
...
like		$@, $answer_ref->[$position++],
							"... and check for the correct error message";
			map{
is			NumberFormats->check( $question_ref->[$position] ), $answer_ref->[$position],
							"Check values against NumberFormat: " . $question_ref->[$position] . " ( test " . $position++ . ")";
			}(0..2);
ok			CustomFormat->check( $question_ref->[$position] ),
							"Check that a good value passes CustomFormat: $question_ref->[$position]";
			$bad_position++;
dies_ok{	CustomFormat->( $bad_value_ref->[$bad_position] ) }
							"Check that a bad value fails CustomFormat: $bad_value_ref->[$bad_position]" ;
like		$@, $answer_ref->[$position],
							"... and check for the correct error message";
explain 								"...Test Done";
done_testing();
